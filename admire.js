...
function renderAdditions(){const additions=[...buckets.items.filter(item=>item.addedBy===other).map(item=>({at:item.createdAt,icon:item.type==='grocery'?'🛒':'✓',title:item.title,meta:item.done?'done':'on the list'})),...buckets.dates.filter(item=>item.addedBy===other).map(item=>({at:item.createdAt,icon:'✦',title:item.title,meta:`date idea · ${item.vibe||'for later'}`}))].filter(item=>item.at).sort((a,b)=>b.at-a.at).slice(0,6);$('admire-additions-empty').hidden=additions.length>0;$('admire-additions').innerHTML=additions.map(item=>`<article class="admire-feed-row"><span>${item.icon}</span><div><p>${escapeHtml(item.title||'')}</p><small>${escapeHtml(item.meta)} · ${timeAgo(item.at)}</small></div></article>`).join('');}
