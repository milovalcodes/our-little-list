...
import { setupAuthUI, applyViewerTheme, escapeHtml, showFailure } from './ui-helpers.js';
...
$('admire-notes').addEventListener('click',async event=>{const button=event.target.closest('[data-note-reaction]');if(!button)return;button.disabled=true;try{await data.setTo('reactions',`note-${button.dataset.noteReaction}-${viewer}`,{targetType:'note',targetId:button.dataset.noteReaction,by:viewer,to:other,emoji:button.dataset.emoji,createdAt:Date.now()});void data.notify(other,{title:`${personName(viewer)} reacted ${button.dataset.emoji}`,body:'to your note',url:'admire.html',kind:'reaction'});}catch(_){showFailure('that reaction did not stick.','check the internet and try again.');button.disabled=false;}});
...
function renderNotes(){const notes=buckets.notes.filter(note=>note.sender===other||note.from===other).sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)).slice(0,5);$('admire-notes-empty').hidden=notes.length>0;$('admire-notes').innerHTML=notes.map(note=>{const reacts=buckets.reactions.filter(reaction=>reaction.targetType==='note'&&reaction.targetId===note.id).map(reaction=>reaction.emoji).join(' ');return `<article class="admire-feed-row"><span>${{heart:'💛',sun:'☀️',moon:'🌙',star:'✦'}[note.mood]||'💌'}</span><div><p>${escapeHtml(note.body||note.message||'')}</p><small>${timeAgo(note.createdAt)}${reacts?` · ${escapeHtml(reacts)}`:''}</small><div class="quick-reactions"><button type="button" data-note-reaction="${escapeHtml(note.id)}" data-emoji="♡">♡</button><button type="button" data-note-reaction="${escapeHtml(note.id)}" data-emoji="😭">😭</button><button type="button" data-note-reaction="${escapeHtml(note.id)}" data-emoji="🫡">🫡</button><button type="button" data-note-reaction="${escapeHtml(note.id)}" data-emoji="criminal behavior">criminal behavior</button></div></div></article>`;}).join('');}
