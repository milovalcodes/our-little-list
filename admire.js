...
$('admire-notes').addEventListener('click',event=>{const picker=event.target.closest('[data-note-picker]');if(picker){const targetId=picker.dataset.notePicker;const current=findNoteReaction(targetId);openEmojiPicker({current:current?.emoji,onSelect:value=>saveNoteReaction(targetId,value,picker),onRemove:()=>saveNoteReaction(targetId,'',picker)});return;}const button=event.target.closest('[data-note-reaction]');if(button)void saveNoteReaction(button.dataset.noteReaction,button.dataset.emoji,button);});

function findNoteReaction(targetId){return buckets.reactions.find(item=>item.id===`note-${targetId}-${viewer}`||(item.targetType==='note'&&item.targetId===targetId&&item.by===viewer));}
async function saveNoteReaction(targetId,value,button){const id=`note-${targetId}-${viewer}`;const existing=findNoteReaction(targetId);if(button)button.disabled=true;try{if(!value||existing?.emoji===value){if(existing)await data.removeFrom('reactions',existing.id||id);toast('reaction removed');}else{await data.setTo('reactions',id,{targetType:'note',targetId,by:viewer,to:other,emoji:value,createdAt:Date.now()});void data.notify(other,{title:`${personName(viewer)} reacted ${value}`,body:'to your note',url:'admire.html',kind:'reaction'});toast(`reacted ${value}`);}}catch(_){showFailure('that reaction did not stick.','check the internet and try again.');}finally{if(button?.isConnected)button.disabled=false;}}
...
function renderNotes(){const notes=buckets.notes.filter(note=>note.sender===other||note.from===other).sort((a,b)=>(b.createdAt||0)-(a.createdAt||0)).slice(0,5);$('admire-notes-empty').hidden=notes.length>0;$('admire-notes').innerHTML=notes.map(note=>{const mine=findNoteReaction(note.id);return `<article class="admire-feed-row"><span>${{heart:'💛',sun:'☀️',moon:'🌙',star:'✦'}[note.mood]||'💌'}</span><div><p>${escapeHtml(note.body||note.message||'')}</p><small>${timeAgo(note.createdAt)}</small><div class="reaction-controls">${mine?`<button class="reaction-display" type="button" data-note-reaction="${escapeHtml(note.id)}" data-emoji="${escapeHtml(mine.emoji)}" aria-label="Remove your ${escapeHtml(mine.emoji)} reaction"><b>${escapeHtml(mine.emoji)}</b><span>yours · tap to undo</span></button>`:''}<button class="reaction-trigger" type="button" data-note-picker="${escapeHtml(note.id)}">react</button></div></div></article>`;}).join('');}
