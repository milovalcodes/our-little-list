...
function show(note){
  ...
  const popup=document.createElement('aside');popup.className='incoming-note';popup.innerHTML=`<button aria-label="Close">×</button><span>${icons[note.mood]||'💌'}</span><div><small>your person says</small><p></p></div>`;popup.querySelector('p').textContent=note.body;popup.querySelector('button').addEventListener('click',()=>popup.remove());document.body.append(popup);setTimeout(()=>popup.remove(),10000);
}
