...
// the variable straight away: tap it quickly and the memory saved with no photo
// — and the picture then attached itself to whatever you saved next.
...
function render(){
  ...
  if(featuredId&&!featured){featuredId='';$('memory-random').hidden=true;$('memory-random').innerHTML='';}
  else if(featured)$('memory-random').innerHTML=memoryMarkup(featured,true);
}
