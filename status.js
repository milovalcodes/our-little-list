...
$('status-pair').addEventListener('click',async event=>{const button=event.target.closest('[data-react-status]');if(!button)return;button.disabled=true;try{await data.setTo('reactions',`status-${button.dataset.reactStatus}-${viewer}`,{targetType:'status',targetId:button.dataset.reactStatus,by:viewer,to:other,emoji:button.dataset.emoji,createdAt:Date.now()});void data.notify(other,{title:`${personName(viewer)} reacted ${button.dataset.emoji}`,body:'to your status',url:'status.html',kind:'reaction'});}catch(_){showFailure('that reaction did not stick.','check the internet and try again.');button.disabled=false;}});
...
function statusCard(person,item={}){
  ...
  const arrival=item.arrival&&Date.now()-Number(item.arrivalAt||0)<4*3600000?`<p class="status-arrival">↗ ${escapeHtml(item.arrival)}</p>`:'';const reacts=reactions.filter(reaction=>reaction.targetType==='status'&&reaction.targetId===person).map(reaction=>reaction.emoji).join(' ');const reactionButtons=person===other?`<div class="quick-reactions"><button type="button" data-react-status="${person}" data-emoji="♡">♡</button><button type="button" data-react-status="${person}" data-emoji="😭">😭</button><button type="button" data-react-status="${person}" data-emoji="🫡">🫡</button><button type="button" data-react-status="${person}" data-emoji="criminal behavior">criminal behavior</button></div>`:'';
  return `<article class="person-status-card ${person===viewer?'is-me':''}"><div class="status-avatar"><img src="${image}" alt=""><i class="status-dot state-${escapeHtml(item.state||'invisible')}"></i></div><div class="status-person-copy"><div class="status-person-top"><strong>${escapeHtml(name)}</strong>${person===viewer?'<span>you</span>':''}</div><p class="status-presence">${escapeHtml(label)} · ${escapeHtml(item.energy||'functioning')}</p>${arrival}${custom?`<p class="status-custom"><b>${escapeHtml(item.emoji||'✦')}</b><span><small>${escapeHtml(item.category||'currently')}</small>${escapeHtml(item.text)}</span></p>`:'<p class="status-blank">no custom status rn</p>'}${reacts?`<small class="reaction-count">${escapeHtml(reacts)}</small>`:''}${reactionButtons}</div></article>`;
}
