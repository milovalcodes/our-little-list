...
const buckets={items:[],reminders:[],help:[],focus:[]};
...
$('focus-form').addEventListener('submit',async event=>{event.preventDefault();const button=$('focus-start');setButtonBusy(button,true,'starting…');const startedAt=Date.now();const label=$('focus-label').value.trim()||'doing the thing';try{await data.setTo('focus',viewer,{person:viewer,active:true,label,minutes:focusMinutes,startedAt,endsAt:startedAt+focusMinutes*60000,endedAt:0,updatedAt:startedAt});void data.notify(other,{title:`${personName(viewer)} is locking in`,body:`${label} · ${focusMinutes} min`,url:'today.html',kind:'focus'});toast('timer started. extremely brave.');}catch(_){showFailure('the timer did not start.','check the internet and try again.');}finally{setButtonBusy(button,false);}});
$('focus-pair').addEventListener('click',async event=>{const button=event.target.closest('[data-stop]');if(!button||button.dataset.stop!==viewer)return;button.disabled=true;const running=buckets.focus.find(entry=>entry.id===viewer||entry.person===viewer);
  try{await data.setTo('focus',viewer,{person:viewer,active:false,label:running?.label||'doing the thing',endedAt:Date.now(),updatedAt:Date.now()});toast('focus session survived');}catch(_){showFailure('the timer refused to stop.','refresh and try once more.');button.disabled=false;}});
