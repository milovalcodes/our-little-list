...
const buckets={items:[],reminders:[],help:[],focus:[],brainDump:[],statuses:[]};
...
$('dump-form').addEventListener('submit',async event=>{event.preventDefault();const text=$('dump-text').value.trim();if(!text)return;const button=event.currentTarget.querySelector('button');setButtonBusy(button,true,'…');try{await data.addTo('brainDump',{text,addedBy:viewer,createdAt:Date.now()});$('dump-text').value='';void data.notify(other,{title:`${personName(viewer)} parked a thought`,body:text,url:'today.html',kind:'brain-dump'});}catch(_){showFailure('the thought escaped.','check the internet and save it again.');}finally{setButtonBusy(button,false);}});
$('dump-list').addEventListener('click',async event=>{const button=event.target.closest('[data-action]');if(!button)return;const row=button.closest('[data-id]');const item=buckets.brainDump.find(entry=>entry.id===row?.dataset.id);if(!item)return;button.disabled=true;try{if(button.dataset.action==='task'){await data.addTo('items',{title:item.text,type:'task',due:dateKey(new Date()),recurrence:'once',addedBy:viewer,done:false,createdAt:Date.now()});await data.removeFrom('brainDump',item.id);toast('moved to the list');}else if(button.dataset.action==='reminder'){location.href=`reminders.html?prefill=${encodeURIComponent(item.text)}`;}else if(button.dataset.action==='delete'){await data.removeFrom('brainDump',item.id);}}catch(_){showFailure('that thought did not move.','check the internet and try again.');button.disabled=false;}});

function render(){renderToday();renderFocus();renderDump();}
function renderToday(){const today=dateKey(new Date());const end=new Date();end.setHours(23,59,59,999);const due=[
  ...
  ...buckets.help.filter(item=>item.to===viewer&&item.state==='open').map(item=>({id:`help-${item.id}`,icon:item.emoji||'🙋',title:item.title,meta:'needs an answer',href:'help.html'}))
...
function renderDump(){const items=[...buckets.brainDump].sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));$('dump-empty').hidden=items.length>0;$('dump-list').innerHTML=items.map(item=>`<article class="dump-row" data-id="${escapeHtml(item.id)}"><div><strong>${escapeHtml(item.text)}</strong><small>${escapeHtml(personName(item.addedBy))} · ${timeAgo(item.createdAt)}</small></div><div><button type="button" data-action="task">list</button><button type="button" data-action="reminder">alarm</button><button type="button" data-action="delete" aria-label="Delete">×</button></div></article>`).join('');}
