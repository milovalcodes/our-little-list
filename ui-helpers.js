...
export function setupAuthUI(data,user){
  ...
  root.innerHTML=`<section class="auth-gate"><div class="auth-card"><span class="auth-icon">☀︎☾</span><p class="tiny-kicker">just for us</p><h2>sign in</h2><p>your own login — hers is hers, yours is yours.</p><form id="shared-auth-form"><label><span>Email</span><input id="shared-auth-email" type="email" autocomplete="username" required placeholder="your email"></label><label><span>Password</span><input id="shared-auth-password" type="password" autocomplete="current-password" minlength="6" required placeholder="your password"></label><p class="auth-error" id="shared-auth-error" role="alert"></p><button class="primary-action" type="submit">sign in</button><button class="create-space" id="shared-create-space" type="button">first time? make my account</button></form><p class="auth-fine">this phone will remember you.</p></div></section>`;
