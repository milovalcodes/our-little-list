...
export function setupAuthUI(data,user){
  ...
  root.innerHTML=`<section class="auth-gate"><div class="auth-card"><span class="auth-icon">☀︎☾</span><p class="tiny-kicker">our private little universe</p><h2>Come under the same sky</h2><p>Use the same shared login on both phones so everything stays together.</p><form id="shared-auth-form"><label><span>Email</span><input id="shared-auth-email" type="email" autocomplete="username" required placeholder="our@email.com"></label><label><span>Password</span><input id="shared-auth-password" type="password" autocomplete="current-password" minlength="6" required placeholder="at least 6 characters"></label><p class="auth-error" id="shared-auth-error" role="alert"></p><button class="primary-action" type="submit">Enter our little universe</button><button class="create-space" id="shared-create-space" type="button">Create our shared sky</button></form><p class="auth-fine">Create it once, then use the same login on the other phone.</p></div></section>`;
