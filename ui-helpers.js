...
export function setupAuthUI(data,user){
  ...
  root.innerHTML=`<section class="auth-gate"><div class="auth-card"><span class="auth-icon">☀︎☾</span><p class="tiny-kicker">exclusive club. two members.</p><h2>prove it’s u</h2><p>same secret login on both phones. cybersecurity or whatever.</p><form id="shared-auth-form"><label><span>Email</span><input id="shared-auth-email" type="email" autocomplete="username" required placeholder="our@email.com"></label><label><span>Password</span><input id="shared-auth-password" type="password" autocomplete="current-password" minlength="6" required placeholder="the secret one"></label><p class="auth-error" id="shared-auth-error" role="alert"></p><button class="primary-action" type="submit">let me in</button><button class="create-space" id="shared-create-space" type="button">make our tiny account</button></form><p class="auth-fine">the phone should remember. allegedly.</p></div></section>`;
